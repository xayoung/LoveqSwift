...
class LaunchAnimationContoller: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        hugoImageView = UIImageView.init(frame: CGRectMake(50, 50, 120, 172))
        let hugoImg = UIImage.init(named: "hugo")
        hugoImageView!.image = hugoImg
        hugoImageView?.alpha = 0;
        self.view.addSubview(hugoImageView!)
        chiImageView = UIImageView.init(frame: CGRectMake(150, 50, 120, 172))
    ...
    func startAnimation(view: UIView?,delay: NSTimeInterval) {
        
        UIView.animateWithDuration(0.3, delay: delay, options: .TransitionCrossDissolve, animations: {
            view!.alpha = 1;
            view!.transform = CGAffineTransformMakeTranslation(0, 200)
                ...
                view!.layer.pop_addAnimation(animation, forKey: "layerPositionAnimation")
